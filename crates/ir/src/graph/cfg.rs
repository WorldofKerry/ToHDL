...
use super::edge::{BranchEdge, Edge};
...
pub struct CFG {
    pub name: String,
    pub graph: petgraph::stable_graph::StableDiGraph<Box<dyn Node>, Box<dyn Edge>>,
    pub entry: NodeIndex,
}
...
impl CFG {
    ...
    pub fn to_dot(&self) -> String {
        ...
        let mut graph: petgraph::stable_graph::StableDiGraph<NodeWithId, Box<dyn Edge>> =
    ...
    pub fn get_edge(&self, from: NodeIndex, to: NodeIndex) -> Option<&Box<dyn Edge>> {
    ...
    pub fn add_edge(&mut self, from: NodeIndex, to: NodeIndex, edge: Box<dyn Edge>) {
        self.graph.add_edge(
            petgraph::graph::NodeIndex::new(from.into()),
            petgraph::graph::NodeIndex::new(to.into()),
            edge,
    ...
    pub fn insert_node_after<T>(&mut self, node: T, idx: NodeIndex, edge_type: Box<dyn Edge>) -> NodeIndex
    where
        T: Node,
    {
    ...
    pub fn insert_node_before<T>(&mut self, node: T, idx: NodeIndex, edge_type: Box<dyn Edge>) -> NodeIndex
    where
        T: Node,
    {
        let new = self.graph.add_node(node.into()).index().into();
    ...
    pub fn insert_succ<T>(&mut self, node: T, idx: NodeIndex, edge_type: Box<dyn Edge>) -> NodeIndex
    where
        T: Node,
    {
        let new = self.graph.add_node(node.into()).index().into();
    ...
    pub fn rmv_edge(&mut self, from: NodeIndex, to: NodeIndex) -> Box<dyn Edge> {
        let edge_index = self
            .graph
            .find_edge(
                petgraph::graph::NodeIndex::new(from.into()),

...
impl BasicTransform for LowerToFsmV2 {
...
mod tests {
    ...
    fn range() {
        ...
        let mut lower = LowerToFsmV2::default();
        lower.apply(&mut graph);

        write_graph(&graph, lower.name());

        // Write all new subgraphs to files
        for (i, subgraph) in lower.subgraphs.iter().enumerate() {
            write_graph(&subgraph, format!("{}_{}.dot", lower.name(), i).as_str());
        }
    }
